...
const Modal = ({modalClose, userId, memberId,history}) => {
  ...
  .catch(function (error) {
    ...
    return (
        <div className="modal__container" onClick={onCloseModal}>
            <div className="modal">
                {isVisible ? (<><div className="passion-title"><span style={{color:'#497EF1'}}>[루시퍼] </span><span>열정 평가</span></div>
                <div style={{marginBottom:'45px'}}>다음 항목은 하나 당 1°C가 올라갑니다.</div>
                <div className="text1"><input type="checkbox" onClick={onCountPositive}></input><label>시간을 잘 지킨다</label></div>
                <div className="text2"><input type="checkbox" onClick={onCountPositive}></input><label>결석이 전혀 없다</label></div>
                <div className="text3"><input type="checkbox" onClick={onCountPositive}></input><label>스터디에 열정적으로 참여한다</label></div>
                <div className="text4"><input type="checkbox" onClick={onCountPositive}></input><label>과제 수행률이 높다</label></div>
                <div className="text5"><input type="checkbox" onClick={onCountPositive}></input><label>좋은 정보를 잘 공유해준다</label></div>
                <div className="text6"><input type="checkbox" onClick={onCountPositive}></input><label>다음에도 함께 하고 싶은 스터디원이다</label></div>
                ...
                :<><img style={{marginTop:'-150px', marginLeft:'-600px'}} onClick={()=>{setIsVisible(true)}} src={vector}/><div className="passion-title"><span style={{color:'#497EF1'}}>[루시퍼] </span><span>열정 평가</span></div>
